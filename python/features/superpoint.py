...
class SuperPointConfig(object):
    """    Holds default network parameters"""
    def __init__(self):
        # Path to pretrained weights file
        self.weights_path = os.path.join(dirname,'./superpoint_misc/superpoint_v1.pth')
        # Input image height (default: 120)
        self.H = 800
        # Input image width (default:160)
        self.W = 640
        # Non Maximum Suppression (NMS) distance (default: 4)
        self.nms_dist=4
        # Detector confidence threshold (default: 0.015)
        self.conf_thresh=0.2
        # Use cuda GPU to speed up network processing speed (default: False)
        self.cuda=False
...
class SuperPointFrontend(object):
    ...
    def run(self, img):
        ...
        # --- Process points.
        semi_max = np.max(semi, axis=0) # Calculate max for Numerical Stability
        dense = np.exp(semi - semi_max) # Softmax.
