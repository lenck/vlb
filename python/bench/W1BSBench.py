...
class W1BSBench(Benchmark):
    def __init__(self, tmp_feature_dir = './features/', result_dir = './python_scores/'):
        super(W1BSBench,self).__init__(name = 'Baseline Stereo', tmp_feature_dir = tmp_feature_dir, result_dir = result_dir)
    ...
    def evaluate_unit(self, feature_1, feature_2, task):
        ...
        is_correct = (match_matrix[:,0] == match_matrix[:,1]).astype(np.float32)
                ...
                if not get_feature_flag:
                    if detector.csv_flag:
                        ...
                        for i in range(n_patches):
                            patches_set[i,:,:,0] = image.image_data[i*(w): (i+1)*(w), 0:w]
                        descriptor = detector.extract_descriptor_from_patch(patches_set)
