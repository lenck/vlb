...
def ellipse_warp(H, ell_o, method):
    ...
    for i in range(ell.shape[0]):
        if method == 'standard':
            S = np.array([[ell[i,2],ell[i,3],0],[ell[i,3],ell[i,4],0],[0,0,-1]])
            T = np.array([[1,0,ell[i,0]], [0,1,ell[i,1]],[0,0,1]])

            M = np.matmul(np.matmul(np.matmul(np.matmul(H,T),S), T.transpose()), H.transpose())
            M = -M/M[2,2]
            t_ = -M[0:2,[2]]
...
def ellipse_eigen(frames):
    ...
    for i in range(numFrames):
        ...
        eigvec[i,:] = np.array([evecs[0,0],evecs[1,0],evecs[0,1],evecs[1,1]])
...
def ellipse2vggformat(ell, eigVal, eigVec):
    ...
    vggEll[:,0:2] = ell[:,0:2]
    v1byLambda1 = eigVec[:,0:2]/np.repeat(np.finfo(float).eps+eigVal[:,[0]],2,axis=1)
    v2byLambda2 = eigVec[:,2:4]/np.repeat(np.finfo(float).eps+eigVal[:,[1]],2,axis=1)    
    
    A1 = np.sum(np.hstack((v1byLambda1[:,[0]],v2byLambda2[:,[0]]))*np.hstack((eigVec[:,[0]],eigVec[:,[2]])),axis = 1)
    A2 = np.sum(np.hstack((v1byLambda1[:,[1]],v2byLambda2[:,[1]]))*np.hstack((eigVec[:,[0]],eigVec[:,[2]])),axis = 1)
    A4 = np.sum(np.hstack((v1byLambda1[:,[1]],v2byLambda2[:,[1]]))*np.hstack((eigVec[:,[1]],eigVec[:,[3]])),axis = 1)
...
def ellipse_overlap_fast(f1, f2, options):
    ...
    for i2 in range(N2):
        ...
        if opts['frame2frame']:
            ...
            if opts['fix']:
                thr = thr * s
            canOverlap = scipy.spatial.distance.cdist(f2[[i2],0:2], f1[:,0:2], 'euclidean') < thr
            maxOverlap = np.minimum(a2[i2],a1)/np.maximum(a2[i2],a1)*canOverlap
...
def ellipse_overlap_H(geom, fa, fb, options):
    ...
    if opts['cropFrames']:
        bba = np.array([1, 1, geom['ima_size'][1]+1, geom['ima_size'][0]+1])
        bbb = np.array([1, 1, geom['imb_size'][1]+1, geom['imb_size'][0]+1])
        if opts['cropFramesSafetyEdge']:
            ser = opts['cropFramesSafetyEdge']
            bba = bba + np.array([geom['refimsize']*ser, -geom['refimsize']*ser])
            bbb = bbb + np.array([geom['imsize']*ser, -geom['imsize']*ser])
        fa_valid = np.logical_and(ellipse_in_bbox(ella, bba), ellipse_in_bbox(ella_rep, bbb))
        fb_valid = np.logical_and(ellipse_in_bbox(ellb_rep, bba), ellipse_in_bbox(ellb, bbb))
    ...
    ellipsesPairs, ellipsesOverlaps = ellipse_overlap_fast(ellb_rep[fb_valid,:], ella[fa_valid,:], matching_option)
...
def main():
    ...
    feature_1 = np.array([[14.8277397,5.3841510e+02,2.0014350,4.4456816]],dtype = np.float32)
    feature_2 = np.array([[40.2015991,5.2250775e+02,2.3041577,4.4541903]],dtype = np.float32)
    ...
    tcorr, corr_score, info = ellipse_overlap_H(geom, feature_1, feature_2, {'maxOverlapError': 0.5})
