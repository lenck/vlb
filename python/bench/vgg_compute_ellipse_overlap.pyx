...
def vgg_compute_ellipse_overlap(np.ndarray[np.double_t, ndim=2] feat1, np.ndarray[np.double_t, ndim=2] feat2, int common_part):
    cdef np.ndarray[np.double_t, ndim=2] tdesc_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)
    cdef np.ndarray[np.double_t, ndim=2] tover_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)
    cdef np.ndarray[np.double_t, ndim=2] over_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)+100.0
    cdef np.ndarray[np.double_t, ndim=2] mover_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)
    cdef np.ndarray[np.double_t, ndim=2] desc_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)+1000000.0
    cdef np.ndarray[np.double_t, ndim=2] mdesc_out = np.zeros((feat1.shape[0],feat2.shape[0]), dtype = np.double)
    
    cdef np.ndarray[np.double_t, ndim=1] feat1a = np.zeros((feat1.shape[1],), dtype = np.double)
    cdef np.ndarray[np.double_t, ndim=1] feat2a = np.zeros((feat2.shape[1],), dtype = np.double)
    cdef double max_dist, fac, dx, dy, dist, dr, ov, descd, minr, rx, rx2, ry, ry2
    cdef int i, j, maxx, minx, maxy, miny, mina, bua, bna, t1, t2, mini, minj, dnbr
